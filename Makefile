...
ifneq ($(SKIP_DOCKER),true)
	...
	GO_DOCKER_CMD := docker run \
		...
		-e GOCACHE=/workspaces/badgr/.gocache \
		-v $(PROJECT_ROOT):/workspaces/badgr \
		-w /workspaces/badgr \
		$(GO_DEV_IMAGE)
	...
	HELM_IMAGE := brigadecore/helm-tools:v0.3.0
...
DOCKER_IMAGE_NAME := $(DOCKER_REGISTRY)$(DOCKER_ORG)badgr
...
HELM_CHART_NAME := $(HELM_REGISTRY)$(HELM_ORG)badgr
...
build:
	$(KANIKO_DOCKER_CMD) kaniko \
		...
		--dockerfile /workspaces/badgr/Dockerfile \
		--context dir:///workspaces/badgr/ \
		--no-push
...
.PHONY: hack-deploy
hack-deploy:
	helm dep up charts/badgr && \
	helm upgrade badgr charts/badgr \
		--install \
		--create-namespace \
		--namespace badgr \
		--timeout 60s \
